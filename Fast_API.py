...
if __name__ == "__main__":
    ...
    ssl_certfile = os.environ.get("SSL_CERTFILE", "cert/cert.pem")
    ssl_keyfile = os.environ.get("SSL_KEYFILE", "cert/key.pem")
    
    if use_ssl:
        logger.info(f"Starting HTTPS API server on port {port}...")
        uvicorn.run(
            app,
            host="0.0.0.0",
            port=port,
